...
def _pick_label(bet: ValueBet) -> str:
    base = f"{bet.market.value}: {bet.selection}"
    if bet.line is not None:
        base += f" {bet.line}"
    return f"{base} @ {bet.decimal_odds:.2f}"


def _edge_line(bet: ValueBet) -> str:
    model_pct = round(bet.true_probability * 100)
    book_pct = round((1 / bet.decimal_odds) * 100) if bet.decimal_odds > 1 else 0
    return (
        f"{_pick_label(bet)} — model {model_pct}% vs book ~{book_pct}% "
    ...
    def evaluate(
        ...
        if not value_bets:
            ...
            if top_p is not None and top_p.probability >= 0.48:
                ...
                short = f"{mkt}: {top_p.selection}"
                if top_p.line is not None:
                    short += f" {top_p.line}"
                reasons.insert(0, f"Best model lean: {short} (~{round(top_p.probability * 100)}%).")
                reasons.append("Size small — verify live price on Stake.")
                return MatchVerdict(
                    verdict=Verdict.CAUTION,
                    headline=f"CAUTION — {short}",
                    reasoning=reasons,
                    best_bet=short,
        ...
        best_label = _edge_line(best)
        short = _pick_label(best)

        if total_score >= 0.55 and best.expected_value >= 0.04 and avg_conf >= 0.6:
            verdict = Verdict.BET
            headline = f"BET — {short}"
